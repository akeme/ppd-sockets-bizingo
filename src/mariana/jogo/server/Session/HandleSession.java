package mariana.jogo.server.Session;

import mariana.jogo.server.Model.*;
...
public class HandleSession implements Runnable {
	...
	public HandleSession(Socket p1, Socket p2){
		player1 = new Player(Bizingo.PLAYER_ONE.getValue(), p1);
		player2 = new Player(Bizingo.PLAYER_TWO.getValue(), p2);
	...
	public void run() {		
		...
		try{
				...
				while(continueToPlay){
					...
					if(bizingo.isOver())
						player2.sendData(Bizingo.YOU_LOSE.getValue());		//Game Over notification
					...
					if(bizingo.isOver()){
						player1.sendData(Bizingo.YOU_WIN.getValue());
					...
					if(bizingo.isOver()){
						player1.sendData(Bizingo.YOU_LOSE.getValue());		//Game Over notification
					}					
					...
					//IF game is over, break
					if(bizingo.isOver()){
						player2.sendData(Bizingo.YOU_WIN.getValue());
						continueToPlay=false;
						break;
					}
	...
	private void updateGameModel(int from, int to){
		Position fromPosition= bizingo.getPosition(from);
		Position toPosition = bizingo.getPosition(to);
		toPosition.setPlayerID(fromPosition.getPlayerID());
		fromPosition.setPlayerID(Bizingo.EMPTY_POSITION.getValue());
		
		CheckValidMove(fromPosition, toPosition);
	}
...
/*	private void checkCrossJump(Square from, Square to){		
	...
	private void CheckValidMove(Position from, Position to) {
